//Окно игры
let cvs = document.getElementById("canvas"); //Перетаскиваем Canvas из html в JS и getElementById присваемает к id='canvas' переменную cvs
let ctx = cvs.getContext("2d"); // Указываем что canvas в 2D
let body_tag = document.querySelector("body");
let canvas_tag = document.querySelector("canvas");
...
let bird = new Image(); //Объект птица это изображение (которое скоро мы импортируем)
let bg = new Image(); //Задний фон это изображение
let fg = new Image(); //Передний фон это изображение
let pipeUp = new Image(); //Верхнии препядствия это изображения
let pipeBottom = new Image(); //нижнии препядствия это изображения

//Импорт файлов
bird.src = "images/fish/fish0.png"; //Импортируем изображение с птицой
bg.src = "images/water/water0.png"; //Импортируем изображение заднего фона //Импортируем изображение переднего фона
/*fg.src = "fg.png";*/ pipeUp.src = "pipeUp.png"; //Импортируем изображение с верхними препядствиями
...
bg.style.width = "100%";
bg.style.height = "100%";
//body_tag.style.length = '100%';
//body_tag.style.width = '100%';
//canvas_tag.style.width = '100%';
//body_tag.style.background = 'red';
//document.querySelector('body').style.width='100%';
//document.querySelector('body').style.background='red';
// Звуковые файлы
let fly = new Audio(); //Звук прыжка находится под значением fly
let score_audio = new Audio(); //Звук каждого успешно пройденного препядствия находится под значением score

fly.src = "sounds/fly.mp3"; //Импортируем звук в в звуковой объект fly (src это метод для присваивания объектам (звукового или нет) файлы или файл)
score_audio.src = "sounds/score.mp3"; //Импортируем звук в в звуковой объект score

let gap = 120; //Это расстояние между двумя препядствиями (верхним и нижним). Что-бы пица могла пролетать не задивая блоки препядствий
...
// Создание блоков
let pipe = []; //Создаем пустой, ПОКА ПУСТОЙ!!! массив (pipe)

pipe[0] = {
	//создаем один объект в массиве, поэтому указываем что он нулевой(то есть первый но в JS первый является нулевым)
...
let score = 0;
// Позиция птички
let xPos = 10; //Позиция птрицы по x
let yPos = 150; //Позиция птрицы по y
let grav = 2; //Скорость с которой птичка опускается по y (игрику) вниз

function draw() {
	//Создаем функцию draw
	...
	for (let i = 0; i < pipe.length; i++) {
		//в этом цикле мы будем генерировать премятствия для птички
		ctx.drawImage(pipeUp, pipe[i].x, pipe[i].y); //Мы отображаем верхнее препядствие (pipeUp) с помошью метода drawImage

		ctx.drawImage(
			pipeBottom,
			pipe[i].x,
			pipe[i].y + //Мы отображаем нижнее препядствие (pipeBottom) с помошью метода drawImage,
				pipeUp.height +
				gap
		); //но генерация нижнего блока будет принижена на 90 пикселей из-за pipeUp.height + gap (pipeUp - это верхнее препядствие, pipeUp.height - это высота всего верхнего блока, gap - это отступ между верхним и нижним препядствием) и их сложение дает этот отступ
		...
		if (pipe[i].x == 125) {
			//если он начало изночальной позиции линия передвинулать на 125 пикселей то
			pipe.push({
				//появляются в массиве pipe нове объекты(объекты, мотому что после pipe.push и открытия скобки мы открыли еще и фигурную скобку)
		...
		if (
			...
			//При задевании переднего фона
			location.reload(); // Перезагрузка страницы
		}
		//Счет очков
		if (pipe[i].x == 5) {
			//если препядствие начинает выходить с левой стороны экрана значит прица прошла и…
